...
private def l₁_map (l : loop x₀) (h : homotopy f g) : (l₁.map (F l h)).to_fun = (l.map g).to_fun :=
begin
  ext t,
  simp only [path'.map, F, l₁, path'.coe_apply, homotopy_with.to_fun_one, continuous_map.comp_coe, 
             continuous_map.coe_mk, function.comp_app, coe_fn_coe_base],
end

private def l₂_map (l : loop x₀) (h : homotopy f g) : 
  (l₂.map (F l h)).to_fun = ((h.path.inv.trans (l.map f)).trans h.path).to_fun :=
begin
  ext t,
  simp only [path'.map, F, l₂, path'.trans, continuous_map.comp_coe, continuous_map.coe_mk, 
             function.comp_app, path'.coe_apply, if_congr, coe_fn_coe_base],
  split_ifs with h₁ h₂,
...
private def change_of_basepoint_comp_map_eq_map_aux (l : loop x₀) (h : homotopy f g) :=
  path_homotopy.double_swap (homotopy_l₁_l₂.map _) (l₁_map l h) (l₂_map l h)

lemma change_of_basepoint_comp_map_eq_map (h : homotopy f g) : 
...
calc (π₁.map h.inv_fun).comp (π₁.map h.to_fun : π₁ x₀ →* _) = 
  (@π₁.change_of_basepoint _ _ x₀ _ (homotopy.path h.left_inv.symm)).to_monoid_hom.comp 
